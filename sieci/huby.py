...
with open('Human_Interactome_May.sif') as plik:
...
wystapienia = []
liczba_polaczen = []
with open('output1.csv', 'w') as plik:
...
with open('output2.csv', 'w') as plik:
...
with open('output4.csv', 'w') as plik:
    for i in liczba_polaczen_met1:
        plik.write(str(i[1]) + ',\t' + str(i[0]) + '\n')
threshold = mean + 2 * stdev
minimum2 = None
with open('output5.csv', 'w') as plik:
    for i in liczba_polaczen:
        if i[0] >= threshold:
            ...
            break
with open('output3.csv', 'w') as plik:
